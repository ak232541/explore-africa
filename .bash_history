    ...
    EOF }   throw new functions.https.HttpsError('internal', 'Failed to retrieve salesperson data.');able, 'desc').limit(1).get();.reference_id });').doc();e}`);
> 
> ^C
alankowa541@cloudshell:~/my-react-app/functions/src$ cat << 'EOF' > index.ts
...
cat << 'EOF' > index.ts
     ...
     */
    export const createPayPalOrder = functions.https.onCall(async (data: PayPalOrderData, context: functions.https.CallableContext) => {
        functions.logger.info('createPayPalOrder: Function started.', { data, auth: context.auth?.uid });
        ...
        const { selectedTierId, userId } = data;

        // Validate input
        if (!selectedTierId) {
            functions.logger.error('Invalid argument: selectedTierId is missing.', { userId });
        ...
        const paymentTiers: { [key: string]: { currency_code: string; value: string; description: string; } } = {
            'local': { currency_code: 'USD', value: '50.00', description: 'Local (Botswana Citizens) Membership' }, // Use USD as a safe default for PayPal
        ...
        if (!selectedTier) {
            functions.logger.error('Invalid payment tier selected.', { selectedTierId, userId });
                    ...
                    if (userId && (paymentStatus === 'COMPLETED' || paymentStatus === 'APPROVED')) {
                        ...
                        await db.runTransaction(async (transaction) => {
                            ...
                            transaction.update(userRef, {
                                ...
                                paymentTier: selectedTierId || 'unknown', // Attempt to derive tier if not explicit
     ...
     */
    export const getSalespersonData = functions.https.onCall(async (data: any, context: functions.https.CallableContext) => {
...
cd ~/my-react-app/functions
rm -f src/index.ts       # Ensure any old, problematic index.ts is gone
rm -rf lib               # Remove any old compiled JavaScript output
rm -rf node_modules      # Remove all installed packages
rm -f package-lock.json  # Remove the package-lock file
npm install
cat index.ts
ls -l index.ts
cat index.ts
...
firebase functions:config:set paypal.client_id="AdXz9VHuG44Ea89ucxQmO_YOUR_FULL_CLIENT_ID_HERE" paypal.client_secret="EFHzgjEFd9JzTA8Uv8voMXwtvE62UAr3bGmp4X9rTN38aLd5ND0VVNxPsBhRmKOtAGeKENqFvbaeBcS6"
firebase deploy --only functions
cd ~/my-react-app/functions
rm -f src/index.ts
rm -rf lib
...
firebase functions:config:set paypal.client_id="AdXz9VHuG44Ea89ucxQmO_YOUR_FULL_CLIENT_ID_HERE" paypal.client_secret="EFHzgjEFd9JzTA8Uv8voMXwtvE62UAr3bGmp4X9rTN38aLd5ND0VVNxPsBhRmKOtAGeKENqFvbaeBcS6"
firebase deploy --only functions
alankowa541@cloudshell:~/my-react-app/functions$ ls -l index.ts
-rw-rw-r-- 1 alankowa541 alankowa541 17778 Jun 10 03:59 index.ts
cd ~/my-react-app/functions
...
gcloud config set project guided-botswana
gcloud functions add-iam-policy-binding paypalWebhookHandler   --member='allUsers'   --role='roles/cloudfunctions.invoker'   --region='us-central1'
gcloud config set project guided-botswana
gcloud run services describe paypalwebhookreceivernew --region=us-central1
